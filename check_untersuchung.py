"""
Überprüft die Struktur der Tabelle Untersuchung und zeigt Beispieldaten an.
"""
...
if __name__ == "__main__":
    # Spalten der Tabelle Untersuchung anzeigen
    print("=== Spalten der Tabelle Untersuchung ===")
    columns = get_columns("Untersuchung")
    if columns.get("success", False):
        print(json.dumps(columns["results"], indent=4))
    else:
        print(f"Fehler: {columns.get('error', 'Unbekannter Fehler')}")
    
    # Beispieldaten aus der Tabelle Untersuchung anzeigen
    print("\n=== Beispieldaten aus Untersuchung ===")
    sample_data = get_sample_data("Untersuchung")
    if sample_data.get("success", False):
        print(json.dumps(sample_data["results"], indent=4, cls=JSONEncoder))
    else:
        print(f"Fehler: {sample_data.get('error', 'Unbekannter Fehler')}")
    
    # Versuche, eine korrekte Abfrage für die Untersuchungen mit Patientendaten zu erstellen
    print("\n=== Untersuchungen mit Patientendaten ===")
    query = """
        SELECT TOP 5 u.*, p.Nachname, p.Vorname, p.Geburtsdatum, ua.UntersuchungartName
        FROM Untersuchung u
