...
if __name__ == "__main__":
    ...
    latest_untersuchungen = get_latest_untersuchungen(5)
    ...
    # Versuche mit einem bekannten Datum (17.03.2025)
    known_date = "17.03.2025"
    print(f"\n=== Untersuchungen am {known_date} ===")
    known_untersuchungen = get_untersuchungen_by_date(known_date)
    if known_untersuchungen.get("success", False):
        if known_untersuchungen["results"]:
            print(f"Anzahl der Untersuchungen: {len(known_untersuchungen['results'])}")
            print(json.dumps(known_untersuchungen["results"][0], indent=4, cls=JSONEncoder))
            print("...")
        else:
            print(f"Keine Untersuchungen für das Datum {known_date} gefunden.")
    else:
        print(f"Fehler: {known_untersuchungen.get('error', 'Unbekannter Fehler')}")
